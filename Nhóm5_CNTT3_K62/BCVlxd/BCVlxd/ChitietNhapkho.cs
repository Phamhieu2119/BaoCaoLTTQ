...
namespace BCVlxd
{
    public partial class ChitietNhapkho : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    pd.capNhat("Insert into Quanlysanphamkho(Mahoadon,Makho,Mavattu,Soluongnhap,Gianhap) Values(N'" + cmbMaHD.Text+"', N'"
                        +cmbMakho.Text+"', N'"
                        +cmbMaVT.Text+ "','"+int.Parse(txtSoluong.Text.ToString())+"','"+ float.Parse(txtDongianhap.Text)+"')");
                    pd.capNhat("UPDATE q SET q.Ngaynhap = n.Ngaynhap FROM Quanlysanphamkho AS q JOIN Nhapkho AS n ON q.Mahoadon = n.Mahoadon " +
                        "WHERE q.Mahoadon = N'" + cmbMaHD.Text+"'");
                    pd.capNhat("update Quanlysanphamkho set Soluongtonkho = (select c.soluong " + "FROM Vattu v " +
                                    ...
                                    "where Mahoadon = N'" + cmbMaHD.Text+ "' and Makho =N'" + cmbMakho.Text+ "' and Mavattu = N'" + cmbMaVT.Text+"'");
                    pd.capNhat("update Quanlysanphamkho set Giaban = Gianhap *(1.1)");
                    ...
                    pd.capNhat("INSERT INTO Chitietnhapkho (Mahoadon,Mavattu,dongianhap,soluong) "
                        + " values( N'" + cmbMaHD.Text + "', N'" + cmbMaVT.Text + "', '" + float.Parse(txtDongianhap.Text) + "','" + 
                        int.Parse(txtSoluong.Text) + "')");
                    string sql2 = "update Chitietnhapkho set thanhtien = soluong * dongianhap";
                    pd.capNhat(sql2);
                    pd.dongketnoi();
                    pd.ketnoi();
                    string query1 = "select *  from Quanlysanphamkho where Makho = N'" + cmbMakho.Text + "' and Mavattu = N'" + cmbMaVT.Text + "' " +
                        "order by Ngaynhap ";
                    SqlDataAdapter adapter1 = new SqlDataAdapter(query1, pd.Con);
                    DataTable dataTable1 = new DataTable();
                    adapter1.Fill(dataTable1);
                    // Tìm đến hàng cuối cùng của bảng
                    int rowIndex = dataTable1.Rows.Count - 1;

                    // Lấy dữ liệu từ hàng cuối cùng
                    double giaNhap = Convert.ToDouble(dataTable1.Rows[rowIndex]["Gianhap"]);
                    double giaBan = Convert.ToDouble(dataTable1.Rows[rowIndex]["Giaban"]);
                    string sql = "UPDATE v " +
                                    "SET v.Gianhap = '" + giaNhap + "', v.Giaxuat = '" + giaBan + "' " +
                                    "FROM Vattu v " +
                                    "JOIN Chitietkhohang c ON v.Mavattu = c.Mavattu " +
                                    "JOIN Khohang k ON k.Makho = c.Makho " +
                                    "WHERE c.Mavattu = N'" + cmbMaVT.Text + "' AND c.Makho = N'" + cmbMakho.Text + "'";
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                pd.ketnoi();
                string sql9 = "select sum(soluong) from Vattu as v join Chitietkhohang as c on v.Mavattu = c.Mavattu join Khohang as k on k.Makho = c.Makho " +
                    "where c.MaVattu = N'" + cmbMaVT.Text + "'and c.Makho = N'" + cmbMakho.Text + "'";
                ...
                pd.capNhat("Delete Quanlysanphamkho where  Mahoadon = N'"
                + cmbMaHD.Text + "' and Makho = N'" + cmbMakho.Text +
                "' and Mavattu = N'" + cmbMaVT.Text + "'");
                string sql8 = "delete c from Chitietnhapkho as c join Nhapkho as n on c.Mahoadon = n.Mahoadon where c.Mahoadon =N'" + cmbMaHD.Text +
                    "'and c.Mavattu = N'" + cmbMaVT.Text + "' and n.Makho = N'" + cmbMakho.Text + "'";
                pd.capNhat(sql8);
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            if (check())
            {
                ...
                if (MessageBox.Show("Bạn có muốn sửa hóa đơn này không ?", "Warning", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    pd.ketnoi();
                    pd.capNhat("update Quanlysanphamkho set Soluongnhap = '" + int.Parse(txtSoluong.Text) + "' , Gianhap = '" + 
                        float.Parse(txtDongianhap.Text)+"'"+
                        " where Mahoadon = N'" + cmbMaHD.Text+ "' and Makho = N'" + cmbMakho.Text+ "' and Mavattu = N'" + cmbMaVT.Text+"'");
                    pd.capNhat("update Quanlysanphamkho set Giaban = Gianhap *(1.1)");
                    pd.dongketnoi();
                    pd.ketnoi();
                    string query = "select *  from Quanlysanphamkho where Makho = N'" + cmbMakho.Text + "' and Mavattu = N'" + cmbMaVT.Text + "' " +
                        "order by Ngaynhap ";
                    SqlDataAdapter adapter = new SqlDataAdapter(query, pd.Con);
                    DataTable dataTable = new DataTable();
                    adapter.Fill(dataTable);
                    ...
                    // Lấy dữ liệu từ hàng cuối cùng
                    double giaNhap = Convert.ToDouble(dataTable.Rows[rowIndex]["Gianhap"]);
                    double giaBan = Convert.ToDouble(dataTable.Rows[rowIndex]["Giaban"]);
