...
namespace BCVlxd
{
    public partial class QLNhanvien : UserControl
    {
        ...
        {
            dataGridView1.DataSource = pd.docbang("SELECT n.MaNV,n.TenNV,n.Gioitinh,n.Ngaysinh,n.Dienthoai,n.MaCV, c.TenCongViec " +
                " FROM Nhanvien as n JOIN Congviec as c ON n.MaCV = c.MaCV where Not MaNV ='0'");
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        pd.capNhat("INSERT INTO Nhanvien (MaNV,TenNV,Gioitinh,Ngaysinh,Dienthoai,MaCV) "
                            + " values( N'" + txtMaNV.Text + "', N'" + txtTenNV.Text + "', " + (isMale ? "1" : "0") + ", '" + 
                            dateTimePicker1.Value.ToString("yyyy-MM-dd") + "', N'" + txtDienThoai.Text + "', N'" + cmbMaCV.Text + "')");
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            if (check())
            {
                if (MessageBox.Show("Bạn có muốn sửa nhân viên này không", "Warning", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    "',Gioitinh=N'" + isMale.ToString() + "',Ngaysinh = N'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") 
                    + "',MaCV=N'" + cmbMaCV.Text +  "' WHERE MaNV=N'" + txtMaNV.Text + "'";
                    pd.capNhat(sql);
                    dataGridView1.DataSource = pd.docbang("SELECT n.MaNV,n.TenNV,n.Gioitinh,n.Ngaysinh,n.Dienthoai,n.MaCV, c.TenCongViec"+
                        "FROM Nhanvien as n JOIN Congviec as c ON n.MaCV = c.MaCV where Not MaNV ='0'");
                }
